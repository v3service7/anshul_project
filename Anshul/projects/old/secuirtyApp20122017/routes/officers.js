...
router.get('/timestamp', function(req, res, next) {
	...
	officersModel.find({role:'Employee',"timestamp":{$gte:datetime}}, { firstName: 1, nickname: 1, lastName:1, phones: 1, emails: 1 ,residenceAddress:1,hasCar:1,isOwl:1, isActive:1,isContractor:1,inEventsRoster:1,isIndependent:1,ccws:1,exposedPermits:1}).exec(function (err, data) {
		if (err) {
			response = {"error" : true,"message" : "Error fetching data"};
		} else{			
			var officer={};
			...
			officersModel.find({role:'Reject',"timestamp":{$gte:datetime}}, { firstName: 1, nickname: 1, lastName:1, phones: 1, emails: 1 ,residenceAddress:1,hasCar:1,isOwl:1, reason:1,isPermanent:1}).exec(function (err, data) {
				if (err) {
					response = {"error" : true,"message" : "Error fetching data"};
				} else{
					for (var i = 0; i < data.length; i++) {
...
router.delete('/:id',function(req,res){
	...
	officersModel.remove({ _id:req.params.id}) 
           ...
           response = {"error" : true,"message" : err};
	        ...
	        if (data.result.n) 
	           response = {"error" : false,"message" : "Data Deleted Successfully"};
	        	...
	        	response = {"error" : false,"message" : "Data Not found"};
        } 
        res.json(response);
    });
});
