...
router.get('/employees/:id',function(req,res){
	...
	officersModel.findById(req.params.id).select('-reason').select('-isPermanent').populate({ path: 'referredBy', options: { select: { firstName: 1, lastName: 1 } } }).populate({ path: 'residenceAddress.city', options: { select: { cityName: 1, state: 1 } } }).populate({ path: 'mailingAddress.city', options: { select: { cityName: 1, state: 1 } } }).populate('ccws.city').exec(function (err, data) {
		if (err) {
			response = {"error" : true,"message" : "Error fetching data"};
		} else{
...
router.get('/rejects/:id',function(req,res){
	...
	officersModel.findById(req.params.id).select('-isIndependent').select('-inEventsRoster').select('-isContractor').select('-isActive').populate('referredBy').populate({ path: 'residenceAddress.city', options: { select: { cityName: 1, state: 1 } } }).populate({ path: 'mailingAddress.city', options: { select: { cityName: 1, state: 1 } } }).populate('ccws.city').exec(function (err, data) {
		if (err) {
			response = {"error" : true,"message" : "Error fetching data"};
		} else{
			for (var i = 0; i < data.notes.length; i++) {
