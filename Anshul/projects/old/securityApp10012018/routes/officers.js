...
var mymilitarypromises1 = function(id,Mdata){
	officersModel.findOne({_id: id}, function (err, user) {
		user.militaryService._id = Mdata._id;
...
var mypolicepromises1 = function(id,Mdata){
	officersModel.findOne({_id: id}, function (err, user) {
		user.policeService._id = Mdata._id;
	    user.save(function (err) {
	        if(err) {
	            console.error('ERROR!');
	        }
...
var subObjPolice = function(officer, police, cb){
	if(typeof police._id === 'undefined' || police._id === null)
	{      	 	
    	let policedata={};
    	policedata.unitName =police.unitName;
    	policedata.timestamp= officer.timestamp;
    	var policeunit = new policeunitModel(policedata);
		policeunit.save(function(err,Mdata){
			officer.policeService._id = Mdata._id;
			mypolicepromises1(officer._id,Mdata);
			cb(officer)
		});
    }
}

var subObjMilat = function(officer, military, cb){
	if(typeof military._id === 'undefined' || military._id === null)
	{
		st = true;
    	let militarydata={};
    	militarydata.unitName =military.unitName;
    	militarydata.timestamp= officer.timestamp;
    	var militaryunit = new militaryunitModel(militarydata);
		militaryunit.save(function(err,Mdata){
			officer.militaryService._id = Mdata._id;
			mymilitarypromises1(officer._id,Mdata);
			cb(officer)
		});
	}
}

var subObjUpdate = function(officer, military, police, cb){
	if (typeof military === 'undefined'  && typeof police === 'undefined'  ) {
		cb(officer)
	}
	if (typeof military === 'undefined'  && typeof police === 'undefined'  ) {
		cb(officer)
	}
	if((typeof military !== 'undefined' && military._id === null) && (typeof police !== 'undefined' && police._id === null))
	{
		subObjMilat(officer, military,(data)=>{
			subObjPolice(data,police,(data1)=>{
				cb(data1)
			})
		})	
	}
	...
	if(typeof military !== 'undefined' && typeof police === 'undefined')
	{   
		subObjMilat(officer,military,(data1)=>{
			cb(data1)
		})	
	}
	...
	if(typeof military === 'undefined' && typeof police !== 'undefined')
	{
		subObjPolice(officer,police,(data1)=>{
			cb(data1)
		})	
	}
}
...
router.post('/employees',function(req, res){
	...
	{	
	    ...
	    officers.save(function(err, officer){
	    	console.log(req.body.policeService._id);
	    	if(err) {	    		
				...
				subObjUpdate(officer, req.body.militaryService, req.body.policeService,(data)=>{
					response = {"error" : false,"message" : data};
					res.json(response);
				});
	        }        			        
