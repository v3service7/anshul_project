...
router.delete('/promotion/:id',function(req, res){
	var response={};
	promotionModel.model({_id:req.params.id},function(err,data){
		if(err){
			response = {"error" : true,"message" : "Error fetching data"};
		}else{
			response = {"error" : false,"message" : "Deleted Successfully"};
...
/*-------------------------------START PAGE--------------------------------------------------------*/
router.get('/page',function(resq, res, next){
	var response = {};
...
router.get('/page/:id',function(req,res){
	var response = {};
	pageModel.findById(req.params.id).populate('stateId').populate('countryId').exec(function(err, page){
		if (err) {
			response = {"error" : true,"message" : "Error fetching data"};
		} else{
			response = {"error" : false,"message" : data};
...
router.post('/banner',function(req,res){
	var response = {};
	var banner = bannerModel(req.body);
