    		...
    		if(admin.password == req.body.password){
    			...
    			adminModel.findByIdAndUpdate(req.params.id,newObject, function(err, kitchen){
    				if(err){
    					...
    					response = {"error": false, "message": "password change Successfully"};
    				}
    				res.json(response);
    			});
    		}else{
    	...
    	adminModel.find({email:req.body.email},function(err,data){
    		if(err){
    			...
    			if (data.length>0) {
    				...
    				var content ="Link reset Link <a href='http://34.209.114.118:3003/admin/resetpassword/" +data[0]._id +"'>Click Here</a>"
    				send.mail.sendMail({//mail Options
